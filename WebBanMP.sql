...
CREATE TABLE DIACHIGH ( -- địa chỉ giao hàng
	...
	CONSTRAINT PK_DCGH PRIMARY KEY (ID)
)
...
CREATE TABLE PPVC (
	ID INT IDENTITY NOT NULL,
	TEN NVARCHAR(100),
	CONSTRAINT PK_PPVC PRIMARY KEY (ID)
)
CREATE TABLE HOADON (
    ID VARCHAR(10) NOT NULL, -- CREATE AUTO
    NGTAO DATE, -- NGÀY TẠO HÓA ĐƠN
    DONGIA FLOAT, -- TỔNG (SỐ LƯỢNG * ĐƠN GIÁ)
	ID_DC INT REFERENCES DIACHIGH(ID),
	ID_VANCHUYEN INT REFERENCES PPVC(ID),
...
CREATE TABLE PHIEUNHAP(
	ID VARCHAR(20) NOT NULL PRIMARY KEY, -- CREATE AUTO 
    NGTAO DATE, -- NGÀY TẠO HÓA ĐƠN
	THOIGIAN TIME,
	THANHTIEN FLOAT, -- TỔNG (SỐ LƯỢNG * ĐƠN GIÁ),
	TRANGTHAI BIT,
	ID_NV VARCHAR(10) REFERENCES NHANVIEN(ID)
)
CREATE TABLE CHITIETPN(
    ID_PN VARCHAR(20) REFERENCES PHIEUNHAP(ID),
	ID_SP VARCHAR(5) REFERENCES SANPHAM(ID),
    SOLUONG INT, -- SỐ LƯỢNG > 0, số lượng bán
	GIANHAP FLOAT,
...
CREATE TABLE HOADON_OFF(
	ID VARCHAR(10) NOT NULL PRIMARY KEY, -- CREATE AUTO 
    NGTAO DATE, -- NGÀY TẠO HÓA ĐƠN
	THOIGIAN TIME,
	THANHTIEN FLOAT, -- TỔNG (SỐ LƯỢNG * ĐƠN GIÁ)
	ID_NV VARCHAR(10) REFERENCES NHANVIEN(ID),
...
CREATE TABLE CHITIETHD_OFF(
    ID_HD VARCHAR(10) REFERENCES HOADON_OFF(ID),
	ID_SP VARCHAR(5) REFERENCES SANPHAM(ID),
    SOLUONG INT, -- SỐ LƯỢNG > 0, số lượng bán
	GIABAN FLOAT,
	CONSTRAINT PK_CTHD_OFF PRIMARY KEY (ID_HD,ID_SP)
)
...
AS
	BEGIN TRY
		...
		BEGIN
			INSERT HOADON(ID, ID_DC, ID_VANCHUYEN) SELECT @maHD, @iddc, @idPPVC

			-- LẤY MÃ KHÁCH HÀNG
			SELECT @maKH = ID FROM KHACHHANG WHERE ID_TK = (SELECT ID_TAIKHOAN FROM THONGTINTAIKHOAN tttk join TAIKHOAN on tttk.ID_TAIKHOAN=TAIKHOAN.ID WHERE HOTEN = @tenKH and ID_GR = 3 and COTK = @coTK)
...
AS
	BEGIN TRY
		...
		IF EXISTS(SELECT * FROM TAIKHOAN WHERE ID_GR = @IDGR AND USERNAME = @userName AND USERNAME <> '')
			THROW 51000, N'Username đã tồn tại.', 1;

		IF EXISTS(SELECT * FROM THONGTINTAIKHOAN tttk join TAIKHOAN tk 
					on tttk.ID_TAIKHOAN = tk.id
				 WHERE ID_GR = @IDGR AND HOTEN = @hoTen AND USERNAME = '')
				 return;
        ...
        INSERT THONGTINTAIKHOAN(ID, HOTEN, NGSINH, GTINH, EMAIL, SDT, DCHI, ID_TAIKHOAN, COTK)
        VALUES(DBO.fn_autoIDTTND(@ID), UPPER(@hoTen), @ngSinh, @GTINH, @email, @sdt, @dChi, @ID, @coTK)
		SELECT N'SUCCESS' 'Message'
	END TRY
	BEGIN CATCH
		EXEC sp_GetErrorInfo;
...
INSERT PPVC SELECT N'Giao hàng tiêu chuẩn'
INSERT PPVC SELECT N'Giao hàng nhanh'
...
INSERT NHANVIEN
VALUES
('20220606','2022060600001')

-- SELECT * FROM SANPHAM JOIN DONGIA ON SANPHAM.ID = DONGIA.ID_SP -- SHOW
select * from TAIKHOAN
select * from CHITIETHD join HOADON on CHITIETHD.ID_HD=HOADON.id
select * from SANPHAM
select * from DANHMUC
